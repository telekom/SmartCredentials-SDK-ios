...
@class SCSCGCDAsyncReadPacket;
@class SCGCDAsyncWritePacket;
@class SCGCDAsyncSocketPreBuffer;
@protocol SCGCDAsyncSocketDelegate;
...
#endif
#define SCGCDAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define SCGCDAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define SCGCDAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
...
#define SCGCDAsyncSocketLoggingContext 65535


typedef NS_ENUM(NSInteger, SCGCDAsyncSocketError) {
	SCGCDAsyncSocketNoError = 0,           // Never used
	SCGCDAsyncSocketBadConfigError,        // Invalid configuration
	SCGCDAsyncSocketBadParamError,         // Invalid parameter was passed
	SCGCDAsyncSocketConnectTimeoutError,   // A connect operation timed out
	SCGCDAsyncSocketReadTimeoutError,      // A read operation timed out
	SCGCDAsyncSocketWriteTimeoutError,     // A write operation timed out
	SCGCDAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	SCGCDAsyncSocketClosedError,           // The remote peer closed the connection
	SCGCDAsyncSocketOtherError,            // Description provided in userInfo
};
...
@interface SCGCDAsyncSocket : NSObject

/**
 * SCGCDAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, SCGCDAsyncSocket will automatically create it's own socket queue.
...
- (instancetype)initWithDelegate:(nullable id<SCGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<SCGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq;

/**
 * Create SCGCDAsyncSocket from already connect BSD socket file descriptor
**/
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<SCGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError **)error;

#pragma mark Configuration

@property (atomic, weak, readwrite, nullable) id<SCGCDAsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<SCGCDAsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<SCGCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<SCGCDAsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<SCGCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * For accepting incoming connections, this means SCGCDAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means SCGCDAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, SCGCDAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, SCGCDAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of SCGCDAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a SCGCDAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a SCGCDAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that SCGCDAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - SCGCDAsyncSocketManuallyEvaluateTrust
 ...
 *     So SCGCDAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - SCGCDAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default SCGCDAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct SCGCDAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, SCGCDAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by SCGCDAsyncSocket,
 ...
 *
 * - SCGCDAsyncSocketSSLProtocolVersionMin
 * - SCGCDAsyncSocketSSLProtocolVersionMax
 ...
 * 
 * - SCGCDAsyncSocketSSLSessionOptionFalseStart
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     See Apple's documentation for kSSLSessionOptionFalseStart.
 * 
 * - SCGCDAsyncSocketSSLSessionOptionSendOneByteRecord
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     See Apple's documentation for kSSLSessionOptionSendOneByteRecord.
 * 
 * - SCGCDAsyncSocketSSLCipherSuites
 ...
 *
 * - SCGCDAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see SCGCDAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use SCGCDAsyncSocketSSLProtocolVersionMin & SCGCDAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * SCGCDAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * This is because the SCGCD API offers no mechanism to discover a queue's targetQueue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(SCGCDAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(SCGCDAsyncSocket *)sock didAcceptNewSocket:(SCGCDAsyncSocket *)newSocket
 * {
 ...
 *   The faster and more powerful SCGCD API isn't notified properly in this case.
 * 
 ...
 * Example usage:
 * 
 * - (void)socket:(SCGCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
 * {
...
@protocol SCGCDAsyncSocketDelegate <NSObject>
@optional
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(SCGCDAsyncSocket *)sock;
 ...
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(SCGCDAsyncSocket *)sock didConnectToUrl:(NSURL *)url;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)socket:(SCGCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(SCGCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(SCGCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(SCGCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(SCGCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(SCGCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(SCGCDAsyncSocket *)sock;
 ...
 * 
 * Note: If the SCGCDAsyncSocket instance is deallocated while it is still connected,
...
**/
- (void)socketDidDisconnect:(SCGCDAsyncSocket *)sock withError:(nullable NSError *)err;
...
**/
- (void)socket:(SCGCDAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
