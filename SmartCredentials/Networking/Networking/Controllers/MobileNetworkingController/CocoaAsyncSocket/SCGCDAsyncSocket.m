...
#ifndef SCGCDAsyncSocketLoggingEnabled
#define SCGCDAsyncSocketLoggingEnabled 0
#endif

#if SCGCDAsyncSocketLoggingEnabled

// Logging Enabled - See log level below

// Logging uses the CocoaLumberjack framework (which is also SCGCD based).
...
#define LogAsync   YES
#define LogContext SCGCDAsyncSocketLoggingContext
...
#ifndef SCGCDAsyncSocketLogLevel
#define SCGCDAsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = SCGCDAsyncSocketLogLevel;
...
NSString *const SCGCDAsyncSocketException = @"SCGCDAsyncSocketException";
NSString *const SCGCDAsyncSocketErrorDomain = @"SCGCDAsyncSocketErrorDomain";

NSString *const SCGCDAsyncSocketQueueName = @"SCGCDAsyncSocket";
NSString *const SCGCDAsyncSocketThreadName = @"SCGCDAsyncSocket-CFStream";

NSString *const SCGCDAsyncSocketManuallyEvaluateTrust = @"SCGCDAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const SCGCDAsyncSocketUseCFStreamForTLS = @"SCGCDAsyncSocketUseCFStreamForTLS";
#endif
NSString *const SCGCDAsyncSocketSSLPeerID = @"SCGCDAsyncSocketSSLPeerID";
NSString *const SCGCDAsyncSocketSSLProtocolVersionMin = @"SCGCDAsyncSocketSSLProtocolVersionMin";
NSString *const SCGCDAsyncSocketSSLProtocolVersionMax = @"SCGCDAsyncSocketSSLProtocolVersionMax";
NSString *const SCGCDAsyncSocketSSLSessionOptionFalseStart = @"SCGCDAsyncSocketSSLSessionOptionFalseStart";
NSString *const SCGCDAsyncSocketSSLSessionOptionSendOneByteRecord = @"SCGCDAsyncSocketSSLSessionOptionSendOneByteRecord";
NSString *const SCGCDAsyncSocketSSLCipherSuites = @"SCGCDAsyncSocketSSLCipherSuites";
#if !TARGET_OS_IPHONE
NSString *const SCGCDAsyncSocketSSLDiffieHellmanParameters = @"SCGCDAsyncSocketSSLDiffieHellmanParameters";
#endif

enum SCGCDAsyncSocketFlags
{
...
@interface SCGCDAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation SCGCDAsyncSocketPreBuffer
...
/**
 * The SCGCDAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface SCGCDAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(SCGCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation SCGCDAsyncReadPacket
...
/**
 * The SCGCDAsyncWritePacket encompasses the instructions for any given write.
**/
@interface SCGCDAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation SCGCDAsyncWritePacket
...
/**
 * The SCGCDAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface SCGCDAsyncSpecialPacket : NSObject
{
  @public
...
@implementation SCGCDAsyncSpecialPacket
...
@implementation SCGCDAsyncSocket
{
	uint32_t flags;
	uint16_t config;
	
	__weak id<SCGCDAsyncSocketDelegate> delegate;
	...
	SCGCDAsyncReadPacket *currentRead;
	SCGCDAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	SCGCDAsyncSocketPreBuffer *preBuffer;
	...
	SCGCDAsyncSocketPreBuffer *sslPreBuffer;
...
- (id)initWithDelegate:(id<SCGCDAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		delegate = aDelegate;
		...
		preBuffer = [[SCGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<SCGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error {
  return [self socketFromConnectedSocketFD:socketFD delegate:aDelegate delegateQueue:dq socketQueue:NULL error:error];
}

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<SCGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError* __autoreleasing *)error
{
  __block BOOL errorOccured = NO;
  
  SCGCDAsyncSocket *socket = [[[self class] alloc] initWithDelegate:aDelegate delegateQueue:dq socketQueue:sq];
  ...
  dispatch_sync(socket->socketQueue, ^{ @autoreleasepool {
    ...
    if (retVal)
    {
      NSString *errMsg = NSLocalizedStringWithDefaultValue(@"SCGCDAsyncSocketOtherError",
                                                           @"SCGCDAsyncSocket", [NSBundle mainBundle],
      ...
      errorOccured = YES;
      if (error)
        *error = [NSError errorWithDomain:SCGCDAsyncSocketErrorDomain code:SCGCDAsyncSocketOtherError userInfo:userInfo];
      return;
    }
...
- (void)getDelegate:(id<SCGCDAsyncSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create SCGCDAsyncSocket instance for accepted socket
			
			SCGCDAsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:SCGCDAsyncSocketErrorDomain code:SCGCDAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:SCGCDAsyncSocketErrorDomain code:SCGCDAsyncSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"SCGCDAsyncSocketConnectTimeoutError",
	                                                     @"SCGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:SCGCDAsyncSocketErrorDomain code:SCGCDAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"SCGCDAsyncSocketReadMaxedOutError",
														 @"SCGCDAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:SCGCDAsyncSocketErrorDomain code:SCGCDAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"SCGCDAsyncSocketReadTimeoutError",
	                                                     @"SCGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:SCGCDAsyncSocketErrorDomain code:SCGCDAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"SCGCDAsyncSocketWriteTimeoutError",
	                                                     @"SCGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:SCGCDAsyncSocketErrorDomain code:SCGCDAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"SCGCDAsyncSocketClosedError",
	                                                     @"SCGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:SCGCDAsyncSocketErrorDomain code:SCGCDAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:SCGCDAsyncSocketErrorDomain code:SCGCDAsyncSocketOtherError userInfo:userInfo];
}
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the SCGCDAsyncSocketUseCFStreamForTLS flag.
                        ...
                        tag:(long)tag
{
	...
	SCGCDAsyncReadPacket *packet = [[SCGCDAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentRead || ![self->currentRead isKindOfClass:[SCGCDAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[SCGCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued SCGCDAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued SCGCDAsyncReadPacket");
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via SCGCDAsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		SCGCDAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		SCGCDAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	SCGCDAsyncWritePacket *packet = [[SCGCDAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentWrite || ![self->currentWrite isKindOfClass:[SCGCDAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[SCGCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued SCGCDAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued SCGCDAsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a SCGCDAsyncSpecialPacket (startTLS packet)
...
- (void)doWriteTimeout
{
	...
	{
		SCGCDAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	SCGCDAsyncSpecialPacket *packet = [[SCGCDAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			SCGCDAsyncSpecialPacket *tlsPacket = (SCGCDAsyncSpecialPacket *)currentRead;
            NSDictionary *tlsSettings = @{};
            if (tlsPacket) {
...
{
	SCGCDAsyncSocket *asyncSocket = (__bridge SCGCDAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	//  3. SCGCDAsyncSocketSSLPeerID
	//  4. SCGCDAsyncSocketSSLProtocolVersionMin
	//  5. SCGCDAsyncSocketSSLProtocolVersionMax
	//  6. SCGCDAsyncSocketSSLSessionOptionFalseStart
	//  7. SCGCDAsyncSocketSSLSessionOptionSendOneByteRecord
	//  8. SCGCDAsyncSocketSSLCipherSuites
	//  9. SCGCDAsyncSocketSSLDiffieHellmanParameters (Mac)
	//
	...
	{
		NSAssert(NO, @"Invalid value for SCGCDAsyncSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for SCGCDAsyncSocketSSLPeerID."]];
		return;
	}
	...
	{
		NSAssert(NO, @"Invalid value for SCGCDAsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for SCGCDAsyncSocketSSLProtocolVersionMin."]];
		return;
	}
	...
	{
		NSAssert(NO, @"Invalid value for SCGCDAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for SCGCDAsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	...
	{
		NSAssert(NO, @"Invalid value for SCGCDAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for SCGCDAsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	...
	{
		NSAssert(NO, @"Invalid value for SCGCDAsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for SCGCDAsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	...
	{
		NSAssert(NO, @"Invalid value for SCGCDAsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for SCGCDAsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. SCGCDAsyncSocketSSLDiffieHellmanParameters
	...
	{
		NSAssert(NO, @"Invalid value for SCGCDAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for SCGCDAsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use SCGCDAsyncSocketSSLProtocolVersionMin & SCGCDAsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[SCGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"SCGCDAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[SCGCDAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[SCGCDAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("SCGCDAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
+ (void)scheduleCFStreams:(SCGCDAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(SCGCDAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	SCGCDAsyncSocket *asyncSocket = (__bridge SCGCDAsyncSocket *)pInfo;
